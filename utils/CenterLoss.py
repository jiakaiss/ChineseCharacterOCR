...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes, feat_dim, use_gpu=True, trained_path=None):
        ...
        if self.use_gpu:
            self.centers = nn.Parameter(torch.randn(self.num_classes, self.feat_dim, requires_grad=True).cuda())
        else:
            self.centers = nn.Parameter(torch.randn(self.num_classes, self.feat_dim, requires_grad=True))
        
        if trained_path is not None:
            print("----loading pretrained model to centerloss----")
